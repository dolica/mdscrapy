...
class BjstoreSpider(scrapy.Spider):
    name = 'bjstore'
    allowed_domains = ['www.bjrbj.gov.cn']
    start_url = 'http://www.bjrbj.gov.cn/LDJAPP/search/ddyy/ddyy_02_outline_new.jsp?'
    ...
    def parse(self, response):
        ...
        if trs is None or len(trs) <= 3:
            return
        yield from self.parse_table(trs[4:-2])
    ...
    def parse_table(self, datas):
        ...
        for data in datas:
            ...
            yield scrapy.Request('http://www.bjrbj.gov.cn/LDJAPP/search/ddyy/' + aspan, callback=self._parse_detail,
                                 meta={'id': id})

    def _parse_detail(self, response):
        fields = response.css('table[width="700"] tr td>font::text').extract()
        item = ShmedicalItem()
        item['name'] = fields[0]
